  ...
  public async processTransactionSet(input: types.ProcessTransactionSetInput): Promise<types.ProcessTransactionSetOutput> {
    ...
    for (const { txHash, transaction } of input.orderedTransactions) {
      ...
      try {
        ...
        if (!err.expected) {
          logger.error(`Transaction ${txHash.toString("hex")} with payload ${JSON.stringify(transaction)} failed unexpectedly. error: ${err}`);
          throw err;
        } else {
          logger.info(`Transaction ${txHash.toString("hex")} with payload ${JSON.stringify(transaction)} failed. error: ${err}`);
          continue;
        }
      } finally {
